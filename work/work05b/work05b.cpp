...
GLint pointLightLocation ,etaLocation, powerLocation, textureLocation, cameraLocation;
...
int main()
{
  ...
  // we create the application's window
    GLFWwindow* window = glfwCreateWindow(screenWidth, screenHeight, "RTGP_work05b", nullptr, nullptr);
    if (!window)
    {
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        pointLightLocation = glGetUniformLocation(illumination_shader.Program, "pointLightPosition");
        textureLocation = glGetUniformLocation(illumination_shader.Program, "tCube");
        cameraLocation = glGetUniformLocation(illumination_shader.Program, "cameraPosition");
        etaLocation = glGetUniformLocation(illumination_shader.Program, "Eta");
        powerLocation = glGetUniformLocation(illumination_shader.Program, "mFresnelPower");
        ...
        bunnyModelMatrix = glm::translate(bunnyModelMatrix, glm::vec3(0.0f, -2.0f, 0.0f));
        bunnyModelMatrix = glm::rotate(bunnyModelMatrix, glm::radians(orientationY), glm::vec3(0.0f, 1.0f, 0.0f));
        bunnyModelMatrix = glm::scale(bunnyModelMatrix, glm::vec3(0.8f, 0.8f, 0.8f));
        ...
        glUniformMatrix3fv(glGetUniformLocation(skybox_shader.Program, "viewMatrix"), 1, GL_FALSE, glm::value_ptr(view));
        textureLocation = glGetUniformLocation(skybox_shader.Program, "tCube");
        glUniform1i(textureLocation, 0);
        cubeModel.Draw();
        glDepthFunc(GL_LESS); // I reset the depth buffer to default
