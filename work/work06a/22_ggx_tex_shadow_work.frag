...
// Sampling of the DepthMap
float Shadow()
{
    vec3 projCoords = posLightSpace.xyz / posLightSpace.w;
    projCoords = projCoords * 0.5 + 0.5; // ProjCoord between 0 and 1 
    ...
    for (int x = -1; x <=1 ; ++x)
    {
        for (int y = -1; y <= 1; ++y) 
        {
            float pcfDepth = texture(shadowMap, projCoords.xy + vec2(x,y) * texelSize).r; // we only need one channel
