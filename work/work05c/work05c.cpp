...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(screenWidth, screenHeight, "RTGP_work05c", nullptr, nullptr);
    if (!window)
    {
    ...
    // Appraoch we use when we need to run a first pass. This one is for the depth map. 
    // We explicitly say that we consider a depth map and buffer.
    // We also explicitly say to OpenGL to not write any color with GL_NONE
